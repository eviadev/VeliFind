...
export const persistStationStatus = (station: Station) => {
  return from(
...
export const persistStationInfos = (station: Station) => {
...
export const processAndPersistStationStatusesAsTasks = () => {
...
export const processAndPersistStationInfosAsTasks = () => {
  ...
  const worker = new Worker(
    ...
    async job => {
      if (job.name === STATION_PROCESS_STATUS_JOB_NAME) {
        ...
        await lastValueFrom(persistStationStatus(stationStatus));
      }
      ...
      if (job.name === STATION_PROCESS_INFOS_JOB_NAME) {
        ...
        await lastValueFrom(persistStationInfos(stationInfos));
      }
    },
